...
export const CreateForm = ({
  ...
  return (
    <Dialog open={open} onClose={handleClose} sx={styles}>
      <DialogTitle>
        {!edit ? 'Crear Proveedor' : 'Editar Proveedor'}
      </DialogTitle>
      <Form initialValues={initValues} onSubmit={handleSubmit}>
        <DialogTitle className="py-0 text-base">
          {page ? 'Datos proveedor' : 'Datos representante'}
        </DialogTitle>
        ...
        <DialogActions>
          ...
          >
            ...
            <Button onclick={handlePage}>
              {page ? 'Siguiente' : 'Anterior'}
            </Button>
            {!page && <SubmitButton>{!edit ? 'CREAR' : 'EDITAR'}</SubmitButton>}
          </Box>
        </DialogActions>
      </Form>
    </Dialog>
