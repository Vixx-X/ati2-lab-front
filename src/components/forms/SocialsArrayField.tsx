          ...
          <Box className='mt-4'>
            {recursiveGetter(vals, name)?.map((_: any, index: number) => (
              <Box key={index} display="flex" className="my-4 gap-x-4">
                <Field label={t("name of social media")} placeholder={t("Ex: instagram, facebook")} name={`${name}[${index}].name`} />
                <Field label={t("value of social media")} placeholder={t("Ex: @abc123")}  name={`${name}[${index}].value`} />
                <Button
                  onClick={() => {
